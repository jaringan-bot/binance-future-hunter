...
export function registerNativeExtrasTools(server: McpServer): void {
  registerSafeTool(
    ...
    async ({ symbol }) => {
      try {
        const data = await binanceProxy.getFuturesExchangeInfo(symbol);
        const allSymbols = data.symbols ?? [];
        const symbols = symbol ? allSymbols.filter((s) => s.symbol === symbol.toUpperCase()) : allSymbols;
        ...
        if (symbols.length === 0) {
          return {
            content: [
              {
                type: "text" as const,
                text: symbol ? `Symbol ${symbol} tidak ditemukan di exchangeInfo.` : "Tidak ada data exchangeInfo.",
              },
            ],
          };
        }
        ...
        const rows = symbols
          ...
          .map((s) => {
            ...
            return `| ${s.symbol} | ${s.status} | ${s.contractType ?? "-"} | ${(price as { tickSize?: string })?.tickSize ?? "-"} | ${(lot as { minQty?: string })?.minQty ?? "-"} | ${(minNotional as { notional?: string })?.notional ?? "-"} |`;
          })
          .join("\n");
    ...
    async ({ symbol, limit, detail }) => {
      try {
        ...
        if (!trades || trades.length === 0) {
          return { content: [{ type: "text" as const, text: `Tidak ada recent trades untuk ${symbol}.` }] };
        }
  ...
  registerSafeTool(
    ...
    async ({ symbol }) => {
      try {
        ...
        if (list.length === 0) {
          return { content: [{ type: "text" as const, text: "Tidak ada data bookTicker." }] };
        }
  ...
  registerSafeTool(
    ...
    "binance_get_price_ticker",
    {
      ...
      description: "Harga terakhir saja (GET /fapi/v2/ticker/price). Ringan. Symbol opsional (kalau kosong return banyak pair).",
      inputSchema: {
        symbol: symbolSchema.optional(),
      },
      annotations: { readOnlyHint: true, openWorldHint: true },
    ...
    async ({ symbol }) => {
      try {
        ...
        const list = Array.isArray(data) ? data : [data];
        const rows = list.slice(0, 40).map((t) => `| ${t.symbol} | ${fmtPrice(parseFloat(t.price))} |`).join("\n");
  ...
  registerSafeTool(
    ...
    async ({ symbol }) => {
      try {
        ...
        const list = symbol ? allEntries.filter((f) => f.symbol === symbol.toUpperCase()) : allEntries;
  ...
  registerSafeTool(
    ...
    async () => {
      try {
        ...
        const text = [`# Trading Schedule`, ``, `| Symbol/Underlying | Timezone | Sessions |`, `|---|---|---|`, rows].join("\n");
  ...
  registerSafeTool(
    ...
    async ({ symbol, limit, startTime, endTime, detail }) => {
      try {
        ...
        if (list.length === 0) {
          return { content: [{ type: "text" as const, text: `Tidak ada force orders${symbol ? ` untuk ${symbol}` : ""}.` }] };
        }
