...
import { symbolSchema, errorResult, CONTRACT_TYPE_ENUM, FUTURES_DATA_PERIOD_ENUM, detailParam } from "../shared.js";
...
export function registerBasisTools(server: McpServer): void {
  registerSafeTool(
    ...
    "binance_get_basis",
    {
      ...
      inputSchema: {
        symbol: symbolSchema.describe("Pair futures, contoh BTCUSDT. Dikirim ke Binance sebagai param `pair`."),
        contractType: z
        ...
        limit: z.number().int().min(1).max(500).default(30).describe("Jumlah data poin (max 500, default 30)"),
        detail: detailParam,
      },
    ...
    async ({ symbol, contractType, period, limit, detail }) => {
      try {
        ...
        if (!points || points.length === 0) {
          return {
            content: [
              {
                type: "text" as const,
                text: `Tidak ada data basis untuk ${symbol} (${contractType}, period ${period}). Pastikan pair terdaftar di Binance USDS-M Futures.`,
              },
            ],
          };
        }
        ...
        const text = [
          ...
          ``,
          truncated ? `_Menampilkan ${shown.length} terakhir dari ${totalCount} total (avg/tren dihitung dari semua)._` : ``,
          `| Waktu | Basis Rate | Basis | Futures | Index |`,
          `|---|---|---|---|---|`,
          rows,
          ``,
          `_Data LANGSUNG dari Binance GET /futures/data/basis. Beda dari binance_get_basis_history (snapshot D1 futures-vs-spot, watchlist tetap saja). Threshold interpretasi ±0.05% basis rate (sama default basis di binance_get_funding_rate)._`,
        ]
          .filter((line) => line !== "")
          .join("\n");
